...
import { setActiveTab, setPlugins, setTabData } from '../../state/slices/pluginsSLice';
...
const Content = () => {
  ...
  const handleActivateTab = useCallback((tabName: string) => {
  ...
  const handleSwitchPlugin = (plugin: string) => {
    ...
    const body = {
      ...
      [activeTab]: {
        ...
        ...(!pluginIsActive && {
          inactive: tabData[activeTab].inactive.filter(plug => plug !== plugin),
          ...
          {tabPluginsSet.map(plugin => {
            return (
                <Card
                  ...
                  status={active.includes(plugin) || !inactive.includes(plugin) ? "Allowed" : "Blocked"}
                  isDisabled={disabled.includes(plugin)}
                  isChecked={active.includes(plugin) || !inactive.includes(plugin)}
                  isLoading={pluginLoading === plugin}
                  switchDisabled={disabled.includes(plugin) || pluginLoading === plugin}
